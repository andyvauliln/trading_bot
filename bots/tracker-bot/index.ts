...
import { createSellTransaction, fetchAndSaveSwapDetails, getTokenQuotes,getSolanaPrice, calculatePNL } from "./transactions";
...
async function main() {
    try {
        ...
        const solanaPrice = await getSolanaPrice(processRunCounter) || 0;
        for (const [publicKey, privateKey] of walletKeyMap.entries()) {
            ...
            for (const holding of holdings) {
                ...
                if (tokenQuotes.success && tokenQuotes.data && config.sell.auto_sell && config.sell.auto_sell === true) {
                    const calculatedPNL = await calculatePNL(holding, tokenQuotes.data, config.sell.include_fees_in_pnl, solanaPrice);
                    ...
                    if (shouldTakeProfit || shouldStopLoss) {                        
                        const result = await createSellTransaction(holding, tokenQuotes.data, calculatedPNL, processRunCounter, privateKey);
