...
import { createSellTransaction, getTokenQuotes } from "./transactions";
          ...
          if (config.sell.auto_sell && config.sell.auto_sell === true) {
            ...
            if (shouldTakeProfit || shouldStopLoss) {
              ...
              try {
                ...
                // Get token quotes before selling
                const tokenQuotesResponse = await getTokenQuotes(holding, processRunCounter);
                if (!tokenQuotesResponse.success || !tokenQuotesResponse.data) {
                  console.log(`${config.name}|[main]| ⚠️ Failed to get token quotes for ${tokenName}`, processRunCounter);
                  return;
                }

                const calculatedPNL = (tokenCurrentPrice - tokenPerTokenPaidUSDC) * tokenBalance;
                const result = await createSellTransaction(holding, tokenQuotesResponse.data, calculatedPNL, processRunCounter, privateKey);
                ...
                if (txSuccess && txTransaction) {
                  ...
                  const profitLossRecord: ProfitLossRecord = {
                    ...
                    ProfitLossSOLWithFees: Number(Number((tokenCurrentPrice * tokenBalance) - tokenSolPaid - tokenSolFeePaid).toFixed(8)),
                    ProfitLossUSDCWithFees: Number(Number(unrealizedPnLUSDC - (tokenSolFeePaid * solanaPrice)).toFixed(8)),
                    ROIPercentageWithFees: Number(Number(((unrealizedPnLUSDC - (tokenSolFeePaid * solanaPrice)) / (tokenSolPaid * solanaPrice)) * 100).toFixed(2)),
